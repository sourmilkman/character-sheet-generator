...
const resources=await client.listResources();assert.equal(resources.resources.length,1);const widget=await client.readResource({uri:resources.resources[0].uri});assert.equal(widget.contents[0].mimeType,'text/html;profile=mcp-app');assert(String("text" in widget.contents[0] ? widget.contents[0].text : "").includes('CHARACTER SHEET'));
...
const prepared=await client.callTool({name:'prepare_character_sheet',arguments:{project:p}});assert(!prepared.isError);assert.equal(((prepared.structuredContent as {referenceIds:string[]}).referenceIds).length,1);
